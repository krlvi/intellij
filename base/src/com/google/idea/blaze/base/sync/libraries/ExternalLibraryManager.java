...
/**
 * Rebuilds {@link BlazeExternalSyntheticLibrary}s during sync, and updates individual {@link
 * VirtualFile} entries in response to VFS events.
 */
public class ExternalLibraryManager {
  ...
  public ImmutableList<SyntheticLibrary> getLibrary(
    ...
    BlazeExternalSyntheticLibrary library = libraries.get(providerClass);
  ...
  /**
   * Clear this out before sync to avoid spamming {@link VirtualFile#isValid()} errors.
   *
   * <p>They're going to be rebuilt during sync anyway.
   */
  static class StartSyncListener implements SyncListener {
    ...
    public void onSyncStart(Project project, BlazeContext context, SyncMode syncMode) {
      ExternalLibraryManager.getInstance(project).libraries = ImmutableMap.of();
    }
  }

  /** Rebuild during sync to be included in the reindexing operation. */
